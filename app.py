...
# Внимание: импорт Docx2txtLoader из langchain.document_loaders устарел.
# Рекомендуется использовать: from langchain_community.document_loaders import Docx2txtLoader
from langchain.document_loaders import Docx2txtLoader
from langchain_text_splitters import RecursiveCharacterTextSplitter
from langchain_chroma import Chroma
from langchain_gigachat.embeddings import GigaChatEmbeddings

# Для LLM (Deepseek / Hyperbolic)
...
# Разбиваем документ на фрагменты (chunks)
...
# --------------------------------------------------------------------------------
# 6. ИНИЦИАЛИЗАЦИЯ МОДЕЛИ LLM (Deepseek / Hyperbolic)
# --------------------------------------------------------------------------------
logging.info("Инициализируем LLM (ChatOpenAI)")

# Пример с Deepseek (закомментирован):
# llm = ChatOpenAI(
...
# # Инициализация кастомной модели ChatOpenAI (например, через Hyperbolic API)
...
llm = ChatOpenAI(
    api_key='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',
    model="gpt-4o-mini",
    temperature=0,
    streaming=True
)
...
# --------------------------------------------------------------------------------
# 7. ИНИЦИАЛИЗАЦИЯ ПАМЯТИ ДИАЛОГА
# --------------------------------------------------------------------------------
# Память для хранения последних 3 обменов
memory = ConversationBufferWindowMemory(k=3)

# --------------------------------------------------------------------------------
# 8. СИСТЕМНЫЙ ПРОМПТ + ИНСТРУКЦИИ (RAG)
# --------------------------------------------------------------------------------
system_prompt_text = """
...
Если ты не можешь ответить на вопрос на основании контекста, придумай лаконичный ответ с долей юмора, например:
“Сколько стоит килограмм гвоздей?” — "У нас гвозди не продаются, но если вдруг захотите перекусить, у нас есть отличные снэки! За подробностями — добро пожаловать на сайт!😀"
"""

# --------------------------------------------------------------------------------
# 9. ЗАГОТОВКА (PromptTemplate) для RAG с добавлением истории диалога
# --------------------------------------------------------------------------------
template = """
{system_prompt}
...
# --------------------------------------------------------------------------------
# 10. СОЗДАНИЕ ФУНКЦИИ-ЦЕПОЧКИ С ПОДДЕРЖКОЙ ПАМЯТИ (RAG)
# --------------------------------------------------------------------------------
def build_rag_chain(system_text: str):
    """
    Создаёт функцию, которая:
      - Получает вопрос пользователя.
      - Дополняет его историей диалога из памяти (последние 3 обмена).
      - Ищет релевантные документы.
      - Формирует финальный prompt и вызывает LLM.
      - Парсит результат и возвращает ответ.
    """
    def chain_func(inputs: dict):
        ...
        # Используем invoke вместо __call__
        llm_result = llm.invoke(prompt_text)
        # Если результат имеет атрибут content, извлекаем его как строку
        result_text = llm_result.content if hasattr(llm_result, "content") else str(llm_result)
        answer = parser.parse(result_text)
        return answer
    return chain_func
...
# --------------------------------------------------------------------------------
# 11. ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ для разбиения больших ответов
# --------------------------------------------------------------------------------
def chunk_text(text: str, max_size: int = 4096):
    """Разбивает текст на части по не более чем max_size символов."""
    return [text[i: i + max_size] for i in range(0, len(text), max_size)]

# --------------------------------------------------------------------------------
# 12. НАСТРОЙКА БОТА (aiogram 3.7+)
# --------------------------------------------------------------------------------
bot = Bot(
...
# --------------------------------------------------------------------------------
# 13. КНОПКИ (с добавлением "Начать новый диалог")
# --------------------------------------------------------------------------------
def get_keyboard():
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="Сбросить диалог"), KeyboardButton(text="Начать новый диалог")],
        ],
...
# --------------------------------------------------------------------------------
# 14. КОМАНДЫ /start и /reset (с очисткой памяти)
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
# 15. ОБРАБОТКА ВСЕХ ДРУГИХ СООБЩЕНИЙ
# --------------------------------------------------------------------------------
@dp.message()
async def handle_message(message: types.Message):
    ...
    if user_text == "О магазине":
        ...
        await message.answer("Чем могу помочь? Задайте вопрос о товарах или услугах.")
        return
    elif user_text in ["Сбросить диалог", "Начать новый диалог"]:
        await cmd_reset(message)
        return

    # Перед выполнением запроса показываем, что бот "печатает"
    await bot.send_chat_action(chat_id=message.chat.id, action=ChatAction.TYPING)

    # Обработка запроса через RAG-цепочку с учетом памяти
    try:
        response = chain({"question": user_text})
        # Сохраняем текущий обмен в память (память хранит последние 3 обмена)
    ...
    # Отправляем ответ в Telegram, разбивая его на части по 4096 символов
